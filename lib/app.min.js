...
})},_straightAcrossGetContainerSize:function(){return{width:this.straightAcross.x}},_straightAcrossResizeChanged:function(){return!0}},$.fn.imagesLoaded=function(callback){function triggerCallback(){callback.call($this,$images)}function imgLoaded(event){var img=event.target;img.src!==blank&&-1===$.inArray(img,loaded)&&(loaded.push(img),--len<=0&&(setTimeout(triggerCallback),$images.unbind(".imagesLoaded",imgLoaded)))}var $this=this,$images=$this.find("img").add($this.filter("img")),len=$images.length,blank="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==",loaded=[];return len||triggerCallback(),$images.bind("load.imagesLoaded error.imagesLoaded",imgLoaded).each(function(){var src=this.src;this.src=blank,this.src=src}),$this};var logError=function(message){window.console&&window.console.error(message)};$.fn.isotope=function(options,callback){if("string"==typeof options){var args=Array.prototype.slice.call(arguments,1);this.each(function(){var instance=$.data(this,"isotope");return instance?$.isFunction(instance[options])&&"_"!==options.charAt(0)?(instance[options].apply(instance,args),void 0):(logError("no such method '"+options+"' for isotope instance"),void 0):(logError("cannot call methods on isotope prior to initialization; attempted to call method '"+options+"'"),void 0)})}else this.each(function(){var instance=$.data(this,"isotope");instance?(instance.option(options),instance._init(callback)):$.data(this,"isotope",new $.Isotope(options,this,callback))});return this}}(window,jQuery);var Post,Stream,_ref,_ref1,_ref2,_ref3,_ref4,_ref5,_ref6,__hasProp={}.hasOwnProperty,__extends=function(child,parent){function ctor(){this.constructor=child}for(var key in parent)__hasProp.call(parent,key)&&(child[key]=parent[key]);return ctor.prototype=parent.prototype,child.prototype=new ctor,child.__super__=parent.prototype,child},__bind=function(fn,me){return function(){return fn.apply(me,arguments)}};document.gignal={views:{}},Post=function(_super){function Post(){return _ref=Post.__super__.constructor.apply(this,arguments)}return __extends(Post,_super),Post.prototype.idAttribute="stream_id",Post}(Backbone.Model),Stream=function(_super){function Stream(){return this.update=__bind(this.update,this),_ref1=Stream.__super__.constructor.apply(this,arguments)}return __extends(Stream,_super),Stream.prototype.model=Post,Stream.prototype.calling=!1,Stream.prototype.parameters={cid:0,limit:5,sinceTime:0},Stream.prototype.initialize=function(){return this.on("add",this.inset,this),this.update()},Stream.prototype.inset=function(model){var view;switch(model.get("type")){case"text":view=new document.gignal.views.TextBox({model:model});break;case"photo":view=new document.gignal.views.PhotoBox({model:model})}return document.gignal.widget.$el.prepend(view.render().el).isotope("reloadItems").isotope({sortBy:"original-order"})},Stream.prototype.parse=function(response){return response.stream},Stream.prototype.update=function(){var sinceTimeCall,_this=this;if(!this.calling)return this.calling=!0,sinceTimeCall=this.parameters.sinceTime,this.fetch({remove:!1,cache:!0,timeout:15e3,jsonpCallback:"callme",data:{limit:this.parameters.limit,sinceTime:this.parameters.sinceTime,cid:this.parameters.cid+=1},success:function(){return _this.calling=!1,document.gignal.widget.refresh(),_this.parameters.sinceTime=_.max(_this.pluck("created_on")),sinceTimeCall!==_this.parameters.sinceTime?_this.parameters.cid=0:void 0},error:function(c,response){return"timeout"===response.statusText?_this.calling=!1:void 0}})},Stream.prototype.setIntervalUpdate=function(){return window.setInterval(function(){return document.gignal.stream.update()},4500)},Stream}(Backbone.Collection),document.gignal.views.Event=function(_super){function Event(){return this.refresh=__bind(this.refresh,this),_ref2=Event.__super__.constructor.apply(this,arguments)}return __extends(Event,_super),Event.prototype.el="#gignal-widget",Event.prototype.columnWidth=250,Event.prototype.isotoptions={itemSelector:".gignal-outerbox",layoutMode:"masonry",sortAscending:!0},Event.prototype.initialize=function(){var columnsAsInt,mainWidth,radix;return radix=10,mainWidth=this.$el.innerWidth(),columnsAsInt=parseInt(mainWidth/this.columnWidth,radix),this.columnWidth=this.columnWidth+(parseInt((mainWidth-columnsAsInt*this.columnWidth)/columnsAsInt,radix)-1),this.$el.isotope(this.isotoptions)},Event.prototype.refresh=function(){var _this=this;return this.$el.imagesLoaded(function(){return _this.$el.isotope(_this.isotoptions)})},Event}(Backbone.View),document.gignal.views.Text=function(_super){function Text(){return this.render=__bind(this.render,this),_ref3=Text.__super__.constructor.apply(this,arguments)}return __extends(Text,_super),Text.prototype.tagName="p",Text.prototype.className="gignal-text",Text.prototype.re_links=/(\b(https?):\/\/[\-A-Z0-9+&@#\/%?=~_|!:,.;]*[\-A-Z0-9+&@#\/%=~_|])/g,Text.prototype.render=function(){var text;return text=this.model.get("text"),text=text.replace(this.re_links,'<a href="$1" target="_top">link</a>'),this.$el.html(text),this},Text}(Backbone.View),document.gignal.views.TextBox=function(_super){function TextBox(){return this.render=__bind(this.render,this),_ref4=TextBox.__super__.constructor.apply(this,arguments)}return __extends(TextBox,_super),TextBox.prototype.tagName="blockquote",TextBox.prototype.className="gignal-outerbox",TextBox.prototype.initialize=function(){return this.text=new document.gignal.views.Text({model:this.model}).render(),this.footer=new document.gignal.views.Footer({model:this.model}).render()},TextBox.prototype.render=function(){return this.$el.data("saved_on",this.model.get("saved_on")),this.$el.css("width",document.gignal.widget.columnWidth),this.$el.html(this.text.$el),this.$el.append(this.footer.$el),this},TextBox}(Backbone.View),document.gignal.views.PhotoBox=function(_super){function PhotoBox(){return this.render=__bind(this.render,this),_ref5=PhotoBox.__super__.constructor.apply(this,arguments)}return __extends(PhotoBox,_super),PhotoBox.prototype.tagName="blockquote",PhotoBox.prototype.className="gignal-outerbox gignal-imagebox",PhotoBox.prototype.initialize=function(){return this.footer=new document.gignal.views.Footer({model:this.model}).render()},PhotoBox.prototype.render=function(){return this.$el.data("saved_on",this.model.get("saved_on")),this.$el.css("width",document.gignal.widget.columnWidth),this.$el.css("background-image","url("+this.model.get("thumb_photo")+")"),this.$el.append(this.footer.$el),this},PhotoBox}(Backbone.View),document.gignal.views.Footer=function(_super){function Footer(){return this.render=__bind(this.render,this),_ref6=Footer.__super__.constructor.apply(this,arguments)}return __extends(Footer,_super),Footer.prototype.tagName="div",Footer.prototype.className="gignal-box-footer",Footer.prototype.initialize=function(){return this.serviceImg=new Backbone.View({tagName:"img",attributes:{src:"images/"+this.model.get("service")+".png",alt:"Service"}}),this.avatar=new Backbone.View({tagName:"img",className:"gignal-avatar",attributes:{src:this.model.get("user_image"),alt:"Avatar"}}),this.serviceProfileLink=new Backbone.View({tagName:"a",attributes:{href:"http://"+this.model.get("service")+".com/"+this.model.get("username")}})},Footer.prototype.render=function(){return $(this.serviceProfileLink.$el).append(this.avatar.$el),$(this.serviceProfileLink.$el).append(this.model.get("name")),this.$el.html(this.serviceImg.$el),this.$el.append(this.serviceProfileLink.$el),this},Footer}(Backbone.View),jQuery("head").append(jQuery('<link rel="stylesheet" type="text/css" />').attr("href","/lib/style.min.css")),jQuery(function($){return Backbone.$=$,document.gignal.widget=new document.gignal.views.Event,document.gignal.stream=new Stream([],{url:"http://dev.gignal.com/event/api/uuid/"+$("#gignal-widget").data("eventid")+"?callback=?"})});
/*
